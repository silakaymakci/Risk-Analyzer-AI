...
with st.sidebar:
    ...
    st.markdown("---")
    # Hızlı Modlar
    mod = st.radio("🚀 Bir Analiz Modu Seç:", 
                   ["Özel Seçim (Kendim Belirleyeyim)", "Güvenli Liman (Hisse + Altın)", "Kripto Ağırlıklı (Riskli)"])
    
    # Modlara göre varsayılan varlıkları ayarla
    ...
    st.markdown("---")
    st.write(f"✅ **Durum:** {len(secilenler)} varlık analize hazır.")
    baslat = st.button("ANALİZİ BAŞLAT")

# --- 3. ANALİZ MOTORU VE O SAMİMİ AÇIKLAMALAR ---
if baslat:
    if len(secilenler) < 2:
